          ...
          <div className="relative mt-20">
            <div className="relative bg-white/70 backdrop-blur-sm rounded-[60px] p-8 lg:p-12 shadow-xl border border-stone-200/50">
                ...
                <div className="space-y-12">
                      ...
                      {services.map((service) =>(
                        ...
                        >
                          <div className="flex items-center justify-between">
                            <div>
                              <div className="font-light text-base">{service.name}</div>
                              <div className={`text-xs mt-1 ${selectedService === service.name ? "text-stone-300" : "text-stone-500"}`}>
                                {service.duration} <span className="w-px h-3 bg-stone-400  inline-block mx-2 "></span> {service.price}
                              </div>
                            </div>
                            {selectedService === service.name && (
                              <div className="w-5 h-5 bg-emerald-400/80 rounded-full shadow-lg flex items-center justify-center"><Check className=" text-white w-4 h-4 "/></div>
                            )}
                  ...
                  {selectedService && (
                    <div>
                      ...
                      <div className="space-y-3">
                        {specialists.filter(spec => spec.specialties.includes(selectedService)).map((specialist) => (
                          <button
                            key={specialist.id}
                            type="button"
                          ...
                          >
                            <div className="flex items-center gap-4">
                              ...
                              <div className="flex-1">
                                <div className="font-light text-base">{specialist.name}</div>
                                <div className={`text-xs mt-1 ${selectedSpecialist === specialist.name ? "text-stone-300" : "text-stone-500"}`}>{specialist.title}</div>
                              </div>
                            </div>
                            {selectedSpecialist === specialist.name && (
                              <div className="w-5 h-5 bg-emerald-400 rounded-full flex items-center justify-center shrink-0"><Check className="text-white w-4 h-4"/></div>
                            )}
                          </button>
                        ))}
                      </div>
              ...
              <div className="mt-12 flex justify-center">
                <button
                  type="submit"
                  disabled={!selectedService || !selectedSpecialist || !selectedDate || !selectedTime || !fullName || !phoneNumber || !email || !!emailError}
                  className={`px-12 py-5 rounded-[30px] text-sm uppercase tracking-wider font-medium transition-all shadow-lg ${
                    selectedService && selectedSpecialist && selectedDate && selectedTime && fullName && phoneNumber && email && !emailError
